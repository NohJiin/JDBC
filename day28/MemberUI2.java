	...
	public static void main(String[] args) {
		...
		btn_find.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// DAO = 값을 오라클로 전송하는 것
				// VO = 데이터(자바에서)를 오라클로 전달하기 위한 데이터 가방(전송용)을 만드는 것
				MemberDAO3 dao = new MemberDAO3();
				MemberVO bag = dao.one(id);	// MemberVo
				if (bag != null) {
					text_pw.setText(bag.getPw());
					text_name.setText(bag.getName());
					text_tel.setText(bag.getTel());
					text_pw.setBackground(Color.pink);
					text_name.setBackground(Color.pink);
					text_tel.setBackground(Color.pink);
				} else {
